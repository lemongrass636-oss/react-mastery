    ...
    <div className="max-w-2xl mx-auto space-y-6 pb-12">
      ...
      <div className="space-y-4">
        {posts.map((post) => {
          ...
          const isLikedByMe = user?.email ? likedUsers.includes(user.email) : false;

          // 削除権限の判定（デバッグ表示は消しましたが、ロジックは強力なままです）
          const canDelete = user?.email && post.authorId && user.email === post.authorId;

          return (
            <div key={post.id} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm group transition-all hover:shadow-md">
              <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start">
                    ...
                    {canDelete && (
                      <button
                        onClick={() => window.confirm('削除しますか？') && deletePost(post.id)}
                  ...
                  <div className="mt-4 flex items-center gap-6">
                    ...
                    >
                      <Heart className={`h-4 w-4 transition-all active:scale-150 ${isLikedByMe ? 'fill-pink-500 text-pink-500' : 'group-hover/like:scale-110'}`} />
